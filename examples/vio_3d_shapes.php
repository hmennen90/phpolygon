...
/**
 * PHPolygon - VIO 3D shapes showcase
 *
 * All built-in procedural geometry generators rendered through VioRenderer3D:
 *   Box, Sphere, Cylinder, Wedge, Plane (ground)
 *
 * Run: php examples/vio_3d_shapes.php
 */
...
$engine->onInit(function () use ($engine, &$shapeEntities): void {
...
$engine->onUpdate(function (Engine $engine, float $dt) use (&$rotationAngle, &$shapeEntities): void {
    $rotationAngle += $dt * 0.5;

    foreach ($shapeEntities as $entity) {
        $t = $entity->get(Transform3D::class);
        if ($t !== null) {
            $t->rotation = Quaternion::fromAxisAngle(new Vec3(0.0, 1.0, 0.0), $rotationAngle);
        }
    }
});
